...
async def join_channel():
    try:
        await Ze(JoinChannelRequest("@B_G_GG"))
        await Ze(JoinChannelRequest("@T33TD"))
...
ownerhson_id = 6581896306
@Ze.on(events.NewMessage(outgoing=False, pattern='فزعه'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply('هنا يمك سيد ابشر  @B_G_GG')
...
async def _(event):
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await Ze.send_message(event.chat_id, f"لايوجد قنوات  في البوت | SY")
                break
            ...
            try:
                ...
                await Ze.send_message(event.chat_id, f"خطأ من المحتمل تم حظر الانضمام ")
                break
...
@Ze.on(events.NewMessage(outgoing=True, pattern=r"\.تجميع الجوكر"))
async def _(event):
        await event.edit("حسنا, تأكد من انك مشترك ب قنوات الاشتراك الاجباري لتجنب الأخطأء")
        channel_entity = await Ze.get_entity(bot_usernamee)
        await Ze.send_message('@A_MAN9300BOT', 'جاري التجميع بواسطة | SOMY TEAM')
        channel_entity = await Ze.get_entity(bot_usernamee)
        await Ze.send_message('@A_MAN9300BOT', '/start')
        await asyncio.sleep(5)
        msg0 = await Ze.get_messages('@A_MAN9300BOT', limit=1)
        await msg0[0].click(2)
        await asyncio.sleep(5)
        msg1 = await Ze.get_messages('@A_MAN9300BOT', limit=1)
        await msg1[0].click(0)
        ...
        for i in range(100):
            ...
            try:
                try:
                    ...
                    bott = url.split('/')[-1]
                    await Ze(ImportChatInviteRequest(bott))
                msg2 = await Ze.get_messages('@A_MAN9300BOT', limit=1)
                await msg2[0].click(text='تحقق')
                chs += 1
...
async def _(event):
      await event.reply("""السـورس يعمـل احمد علاء يعمل بنجاح @B_G_GG
╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍


- سورس بسيط يحتوي على الاوامر المهمة التي تحتاجها 
- للدخول لقناة المطور تفضل - @B_G_GG.
╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍╍"""
)

@Ze.on(events.NewMessage(outgoing=True, pattern=".مطور"))
async def _(event):
      await event.reply("""توفي العامري : @EEc5e"""
)
